...
def get_int_dev():
    return torch.tensor([0], device="cuda", dtype=torch.int32)

def get_bool_dev():
    return torch.tensor([0], device="cuda", dtype=torch.bool)
...
# 将float、int、bool类型的张量转换为对应的设备类型
def devF(tensor: torch.Tensor):
    return tensor.to(get_float_dev())

def devI(tensor: torch.Tensor):
    return tensor.to(get_int_dev())

def devB(tensor: torch.Tensor):
    return tensor.to(get_bool_dev())
