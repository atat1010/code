...
class Feat_Cat_Net(nn.Module):
    
    def __init__(self, config, input_ch=3, geo_feat_dim=15, hidden_dim=64, num_layers=2):
        super(Feat_Cat_Net, self).__init__()
        self.config = config
        self.input_ch = input_ch
        self.geo_feat_dim = geo_feat_dim
        self.hidden_dim = hidden_dim
    ...
    def get_model(self, tcnn_network=False):
        if tcnn_network:
            ...
            for l in range(self.num_layers):
                ...
                feat_cat_net.append(nn.Linear(in_dim, out_dim, bias=False))
                # if l != self.num_layers - 1:
                feat_cat_net.append(nn.ReLU(inplace=True))

            return nn.Sequential(*nn.ModuleList(feat_cat_net))
...
class Decoders(nn.Module):
    ...
    def __init__(self, config, input_ch=3, input_ch_pos=12, learnable_beta=True):
        ...
        self.w_fusion = 0.001  # weight for feature fusion
        ...
        self.feat_cat_net = Feat_Cat_Net(config,
                              input_ch=64,
                              geo_feat_dim=config['decoder']['geo_feat_dim'],
                              hidden_dim=64,
                              num_layers=config['decoder']['num_layers_cat'])
        self.feat_fusion_net = Feat_Cat_Net(config,
                              input_ch=48,
                              geo_feat_dim=config['decoder']['geo_feat_dim'],
                              hidden_dim=16,
                              num_layers=config['decoder']['num_layers_cat'])
        # if self.type == "Replica":
        #     self.map_to_higher_frequency = Feat_Cat_Net(config,
        #                         input_ch=127,
        #                         geo_feat_dim=config['decoder']['geo_feat_dim'],
        #                         hidden_dim=256,
        #                         num_layers=config['decoder']['num_layers_cat'])
    ...
    def sample_plane_feature(self, p_nor, planes_xy, planes_xz, planes_yz):
        ...
        for i in range(len(planes_xy)):
            ...
            total = torch.cat([xy, xz, yz], dim=-1)     # 48 dim
            feat_fusion = self.feat_fusion_net(total)   # 16 dim
            feat.append(self.w_fusion * feat_fusion + xy + xz + yz)  # 16 dim
            # feat.append(xy + xz + yz)  # 16 dim
        feat = torch.cat(feat, dim=-1)
        feat = self.feat_cat_net(feat)   #Optional
    ...
    def forward(self, p, embed_pos, all_planes):
        ...
        h = self.sdf_net(torch.cat([feat, embed_pos], dim=-1), return_geo=True)
        sdf, geo_feat = h[..., :1], h[..., 1:]
        feat_color = torch.cat([embed_pos, c_feat, geo_feat], dim=-1)
        # if self.type == "Replica":
        #     feat_color = self.map_to_higher_frequency(feat_color)
        rgb = self.color_net(feat_color)
        #sdf = torch.tanh(sdf)
