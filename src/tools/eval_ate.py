...
def evaluate_ate(first_list, second_list, output, plot="", _args=""):
    ...
    if args.plot:
        ...
        plt.savefig(args.plot, dpi=600, transparent=True, bbox_inches='tight', pad_inches=0)
...
if __name__ == '__main__':
    ...
    """
    ckpt = torch.load('output/TUM_RGBD/rgbd_dataset_freiburg1_desk/ckpts/00591.tar', map_location=torch.device('cpu'))
    ...
    scale = cfg['scale']
    output = cfg['save_path'] if args.output is None else args.output

    ckptsdir = f'{output}/ckpts'
    npydir = f'{output}/save_traj'
    if os.path.exists(ckptsdir):
        ...
        if len(ckpts) > 0:
            ...
            # pose_es = np.load(f"{npydir}/pose_es.npy")  # shape: (N, 4, 4)
            # pose_es = [flip_axes(torch.from_numpy(i).float()) for i in pose_es]
            estimate_c2w_list = ckpt['estimate_c2w_list']
            # estimate_c2w_list = pose_es
            ...
            # pose_gt = np.load(f"{npydir}/pose_gt.npy")  # shape: (N, 4, 4)
            # pose_gt = [flip_axes(torch.from_numpy(pose_gt[i]).float()) for i in range(len(pose_gt))]
