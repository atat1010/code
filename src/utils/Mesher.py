...
class Mesher(object):
    ...
    def get_grid_uniform(self, resolution):
        ...
        # print("[DEBUG] marching_cubes_bound:", bound)
        ...
        # print("np.linspace is from:", np.linspace.__module__)
        # import traceback
        # print("===[DEBUG] get_grid_uniform called===")
        # print("type(x):", type(x))
        # print("Call stack:")
        # traceback.print_stack(limit=5)
        # print(f"[DEBUG] nsteps_x: {nsteps_x}, nsteps_y: {nsteps_y}, nsteps_z: {nsteps_z}")
        # print(f"[DEBUG] total points to generate: {nsteps_x * nsteps_y * nsteps_z}")
        # print("x_type: ", type(x), "y_type: ", type(y), "z_type: ", type(z))
        # print("x, y, z", x, y, z)
    ...
    def get_mesh(self, mesh_out_file, all_planes_list, decoders_list, world2rf_paramlist, keyframe_dict,
        ...
        """
        print('get mesh: ', datetime.datetime.now())
        ...
        with torch.no_grad():
            ...
            bbox_min, bbox_max = mesh_bound.bounds  # 两个 (3,) ndarray
            for pnts in torch.split(points, self.points_batch_size, dim=0):
                pnts_np = pnts.cpu().numpy()
                mask_np = np.all((pnts_np >= bbox_min) & (pnts_np <= bbox_max), axis=1)
                mask.append(mask_np)
            ...
            z[~mask] = -1
            ############################################
            # z[~mask] = 1.0     #modify for mesh generation
            # # print('sdffinish', datetime.datetime.now())
            # # print(f"[DEBUG] TSDF values after eval: min={z.min():.4f}, max={z.max():.4f}, mean={z.mean():.4f}")
            # if np.all(z < self.level_set) or np.all(z > self.level_set):
            #     print(f"[WARNING] TSDF volume does not cross level_set={self.level_set}. Skipping mesh extraction.")
            #     return
            ############################################
            try:
                if version.parse(
