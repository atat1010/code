"""
包含各种工具函数包括3D点云处理、图像处理、几何变换、深度图计算、法线计算、特征提取等
"""
...
# 将像素掩码转换为块掩码（用最大池化的方法）取最大值作为网格的值
# pixelmask: [H, W]
# stride: 采样步长
# tile_mask: [H/stride, W/stride]
...
# 将像素掩码转换为基于网格的掩码（用平均池化的方法）取平均值作为网格的值再与阈值比较生成二值掩码
