"""
后端：负责地图的构建和优化
    1.地图更新：通过Tracker传递的帧数据更新点云地图
    2.局部优化：对当前帧的点云进行局部优化
    3.全局优化：对关键帧的点云进行全局优化，提升地图的全局一致性
    4.关键帧管理：检测并管理关键帧
    5.与Tracker的通信：接收Tracker传递的帧数据，传递优化后的地图数据
"""
...
class Mapping(object):
    ...
    def mapping(self, frame, frame_map, frame_id, optimization_params):
        ...
        self.gaussians_add(frame)       #将当前帧的点云添加到临时点云
    ...
    def check_keyframe(self, frame, frame_id):
        ...
        curr_rot = frame.R.T
        curr_trans = frame.T
        # 计算当前帧与上一关键帧的旋转和平移差距，超过阈值就标记为关键帧
    ...
    def evaluate_render_range(
        ...
        return render_mask, tile_mask, render_ratio #渲染掩码、图块掩码、渲染比例
    ...
    # 过滤掉与现有不稳定点云重叠的临时点云
    # 通过KNN检测临时点云和现有不稳定点云之间的距离
    # 如果临时点云与现有不稳定点云的距离小于现有不稳定点云的半径，则删除临时点云
    ...
    # 将临时点云附加到稳定点云中
    # 通过投影临时点云到当前帧，计算与稳定点云的距离
    # 如果距离小于阈值，则将临时点云附加到稳定点云中
