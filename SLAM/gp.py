...
# 用于管理和操作基于高斯分布的点云数据
# 该类提供了加载、删除、添加、更新和保存点云数据的功能
# 还包括一些用于计算高斯分布参数的辅助函数
class GaussianPointCloud(object):

    # 激活函数和反激活函数的设置，用于处理高斯分布参数
    ...
    def __init__(self, args) -> None:
        ...
        self._add_tick = devI(torch.empty(0))   # 记录点云的添加时间
        # error counter
    ...
    # 点云参数化--将点云的属性转化为可训练的参数
    # 这里使用了nn.Parameter来定义可训练的参数
    ...
    # 添加新的点云数据，这些点的属性（位置、法线、颜色）会被初始化并储存到点云对象当中
    def add_empty_points(self, xyz, normal, color, time):
        ...
        self.cat(add_params)    #将点及属性添加到当前点云对象中

    # 更新点云数据的几何属性，包括位置、法线、颜色等
    def update_geometry(self, extra_xyz, extra_radius):
        """
        1.将新点与当前点云合并
        2.计算点之间的几何关系，更新点云的缩放因子
        3.删除无效的点
        """
        xyz = self.get_xyz
    ...
    def get_plane(self):
        """
        return plane[N, 3], axis[N, 1]
        两个正交平面的法向量plane0、plane1
        以及对应的缩放因子axis0、axis1
        """
        scales = self.get_scaling
        R = self.get_R
