...
class SLAM():
    ...
    def __init__(self, dataset, cfg, args):

        self.dataset_rtg = dataset
        ...
        os.makedirs(f'{self.output}/save_traj', exist_ok=True)
        ...
        self.depth_map = torch.zeros((self.H, self.W), dtype=torch.float32)
        self.depth_map.share_memory_()
        ...
        manager = mp.Manager()          #通过IPC底层共享不需要手动释放，进程结束后自动释放
    ...
    def cleanup(self):
        del self.estimate_c2w_list
        del self.gt_c2w_list
        del self.idx
        del self.mapping_first_frame
        del self.mapping_idx
        del self.mapping_cnt
        del self.shared_cur_rf_id
        del self.shared_embedpos_fn
